...
![Chrome Manifest V3](https://img.shields.io/badge/Chrome-Manifest%20V3-blue?logo=googlechrome&logoColor=white)
![yt-dlp](https://img.shields.io/badge/Engine-yt--dlp-red)
![FFmpeg](https://img.shields.io/badge/Audio-FFmpeg-green?logo=ffmpeg&logoColor=white)
...
1. **Injeção Nativa na UI do YouTube:** Um botão visualmente integrado ao tema escuro/claro do YouTube.
...
- **Integração Nativa no YouTube:** O botão `🎵 Baixar Áudio` é injetado automaticamente na barra de ações abaixo do player de vídeo (ao lado de Curtir, Compartilhar, etc.) e atualizado dinamicamente em navegações SPA (sem recarregar a página).
- **Cópia Automática da URL:** Ao clicar no botão, a URL limpa de compartilhamento (`https://youtu.be/...`) é copiada instantaneamente para a área de transferência.
...
- **Personalização de Caminhos e Nomes:** Suporte a máscaras de renomeação do yt-dlp (ex: `%(title)s.%(ext)s`, `%(uploader)s`).
...
├── content/
│   ├── content.js             # Monitora o DOM do YouTube e injeta o botão
...
└── native_host/
    ├── com.ytdlp.downloader.json  # Manifesto do host de mensagens nativas
    ├── host.bat               # Ponto de entrada chamado pelo Chrome
    ├── host.ps1               # Script PowerShell que processa mensagens e executa o yt-dlp
    ├── install_host.bat       # Script de registro automático no Windows
    └── uninstall_host.bat     # Script de remoção da chave do registro
```
...
3. **Native Host (`host.bat` + `host.ps1`)** ➡️ Chrome inicia o conector via `stdio`, o script localiza o `yt-dlp` e `ffmpeg` e inicia o download em uma janela de terminal.
...
4. Selecione a pasta raiz deste projeto:
   ```text
   C:\Users\Usuario\Documents\Projetos\Extencao_chrome
   ```
5. A extensão aparecerá na lista. **Copie o ID gerado** (uma sequência de 32 letras, por exemplo: `dakeljjgakbpnmgiiiblinkjlpiegmdh`).
...
3. Quando solicitado, cole o **ID da extensão** que você copiou no passo anterior (ou pressione **Enter** caso coincida com o padrão sugerido).
4. O instalador gravará o caminho absoluto no arquivo `com.ytdlp.downloader.json` e registrará o host no Registro do Windows sob:
   ```text
...
2. Abaixo do título do vídeo, na barra de botões (ao lado de *Curtir*, *Compartilhar*, etc.), clique no botão **🎵 Baixar Áudio**.
...
5. Escolha o modo de execução:
   - **Executar com yt-dlp (Modo Direto):** O terminal abrirá automaticamente e fará o download.
...
- **Status do Conector:** Teste instantâneo de conexão com o script nativo.
- **Formato Padrão:** Defina qual formato deve vir pré-selecionado na modal.
- **Caminho Padrão:** Configure o diretório de download padrão (ex: `%USERPROFILE%\Music\%(title)s.%(ext)s`).
...
- Se instalou recentemente, reinicie o Chrome para recarregar o ambiente.
...
- Basta executar o arquivo `native_host/uninstall_host.bat`. Ele removerá a chave do registro do Windows de forma segura.
